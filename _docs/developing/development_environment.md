---
layout: page
title: Setting Up Your Manifold Development Environment
menu:
  developers:
...
### Setup Postgres, Create a "Manifold" User
...
### Install API Gem Bundle

We install our gems first so we can run `rails secret` \(see the next step below\) without error!

	cd ~/manifold/api
	bundle install

### Set Secret, DB Credentials, and Service URLs in `~/manifold/.env`
...
### Install Manifold CLI and Deployment Dependencies
